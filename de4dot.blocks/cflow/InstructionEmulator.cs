...
namespace de4dot.blocks.cflow {
	public class InstructionEmulator {
		...
		void Emulate_Stsfld(Instruction instr) {
			var val1 = valueStack.Pop();
			FieldDef fie = instr.Operand as FieldDef;
			if (fie != null) {
				if (val1.valueType == ValueType.Int32) {
					if (valueStack.fieldsDictionary.ContainsKey(fie)) {
						valueStack.fieldsDictionary.Add(fie,val1);
					}
					else {
						valueStack.fieldsDictionary[fie] = val1;
					}
				}
			}
		...
		void Emulate_Ldsfld(Instruction instr) {
			...
			FieldDef fie = instr.Operand as FieldDef;
			if (fie != null) {
				if (valueStack.fieldsDictionary.TryGetValue(fie, out Value val)) {
					valueStack.Push(val);
				}
				else {
